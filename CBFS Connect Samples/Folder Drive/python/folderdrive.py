...
def include_trailing_path_delimiter_ex(path, path_delimiter):
    if path == "":
...
def cb_create_file(params: CBFSCreateFileEventParams):
    global opt_folder, context_holder, holder_lock, holder_key_counter
...
def cb_set_file_attr(params: CBFSSetFileAttributesEventParams):
    ...
    try:
        ...
        if context is not None:
            ...
            with context.counter_lock:
                h_file = ctypes.windll.kernel32.CreateFileW(ctypes.c_wchar_p(full_name), FILE_WRITE_ATTRIBUTES,
                                                           FILE_SHARE_ALL, None, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
                                                           None)
                if h_file != 0:
                    res = ctypes.windll.kernel32.SetFileTime(h_file, byref(ctime), byref(atime), byref(wtime))
...
def print_usage():
    print("usage: folderdrive.py [-<switch 1> ... -<switch N>] <mapped folder> <mounting point>\n")
    print("<Switches>")
                ...
                # permitted process
                    ...
                    if i + 1 < len(sys.argv):
                        ...
                        if opt_permitted_process != "":
                            ...
                            continue
                    print(
                        "Process Id or process name must follow the -ps switch. Process Id must be a positive "
                        "integer, and a process name may contain an absolute path or just the name of the executable "
                        "module.")
                    return 1
...
def run_mounting():
    global disk, opt_local, opt_network, opt_icon_path, opt_drv_path, opt_folder, opt_mounting_point
