...
namespace callback.Demos
{
    class fusememdriveDemo
    {
        ...
        private static void CBCacheReadData(object sender, CBCacheReadDataEventArgs e)
        {
            ...
            try
            {
                VirtualFile vfile = (VirtualFile)GCHandle.FromIntPtr(e.FileContext).Target;
            ...
            try
            {
                ...
                if (FindVirtualFile(e.Path, ref vfile))
                {
                    if (e.FileContext == IntPtr.Zero)
                        e.FileContext = vfile.Handle;
                    mCBCache.FileOpen(e.Path, vfile.Size, Constants.PREFETCH_NOTHING, vfile.Handle);
                }
                else
                    e.Result = -ENOENT;
            }
    ...
    public class VirtualFile : IDisposable
    {
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                this.mName = null;
                this.mStream = null;
                GCHandle gch = GCHandle.FromIntPtr(this.mHandle);
                gch.Free();
