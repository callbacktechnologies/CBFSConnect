...
class VirtualFile(object):
    ...
    def __init__(self, name: str, mode: int = stat.S_IFREG):
        try:
            ...
            print("Cannot create virtual file. Error %s" % ex.message)
    ...
    def last_access_time(self, value: float):
    ...
    def last_write_time(self, value: float):
...
def calculate_dir_size(dir: VirtualFile) -> int:
    result = 0
    for file in dir.subfiles:
    ...
    if vfile:
        ...
        try:
            cache.file_open_ex(proper_case_for_cache(params.path), 0, False, 0, 0, PREFETCH_NOTHING,
                               params.file_context)
        except CBFSConnectCbcacheError as ex:
        ...
        if (params.mode & FALLOC_FL_KEEP_SIZE) != FALLOC_FL_KEEP_SIZE:
            ...
            if (new_size > vfile.size):
                ...
                try:
                    fname = proper_case_for_cache(params.path)
                    if cache.file_exists(fname):
                        cache.file_set_size(fname, new_size)
                except Exception as ee:
                    pass
...
def cb_read(params: FUSEReadEventParams) -> None:
    ...
    if vfile:
        ...
        if not (cache.file_read(proper_case_for_cache(params.path), params.offset, buffer, 0, params.size)):
            ...
            cbuf = ctypes.c_uint8 * len(buffer)
...
def install_driver() -> int:
    ...
    except CBFSConnectFuseError as e:
...
def run_mounting() -> int:
    global cache, fuse, opt_local, opt_network, opt_icon_path, opt_drv_path, opt_mounting_point, opt_permitted_process, disk_context
    ...
    except CBFSConnectCbcacheError as e:
